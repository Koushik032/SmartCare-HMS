    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            try:
                ...
                logger.exception("Webhook sending failed: %s", exc)
            ...
            if webhook_sent:
                ...
                messages.success(request, "Your message was saved successfully, but webhook delivery failed.")
...
from .forms import AdminAddPatientForm
from .models import Patient
...
def admin_add_patient(request):
    ...
    form = AdminAddPatientForm(request.POST or None, request.FILES or None)
...
def receptionist_create_appointment(request):
    ...
    # receptionist only own organization available doctors
...
@login_required
def receptionist_toggle_doctor_availability(request, doctor_id):
    if not hasattr(request.user, 'receptionist'):
        return redirect('home')
    ...
    if doctor.hospital_name.strip().lower() != receptionist.organization_name.strip().lower():
        messages.error(request, "You are not allowed to manage this doctor.")
        return redirect('receptionist_doctors')

    doctor.is_available = not doctor.is_available
    doctor.save()

    if doctor.is_available:
        messages.success(request, f"Dr. {doctor.name} is now available.")
    else:
        messages.success(request, f"Dr. {doctor.name} is now unavailable.")
